...
def lstm_train_and_evaluate(units, activation, learning_rate, train_data, test_data, epochs, batch_size):
...
if __name__ == "__main__":
    ...
    file_path = r'C:\3_ProjectsM\FinancialMultitool\source\iwda_table_reversed.csv'
        ...
        for units in lstm_units:
            for activation in lstm_activations:
                for learning_rate in learning_rates:
                    futures.append(
                        executor.submit(
                            lstm_train_and_evaluate,
                            units, activation, learning_rate,
                            train_data, test_data, epochs, batch_size
                        )
                    )

        for future in futures:
            rmse, model = future.result()
            if rmse < best_rmse:
                best_rmse = rmse
    ...
    # Commented out because of calculation error.
    ...
    # xgb_grid_search.fit(np.arange(len(close_prices_AAPL_reverse)).reshape(-1, 1), close_prices_AAPL_reverse)    # error
    ...
    model_names = ['Support Vector Machines (SVM)', 'Random Forest', 'LightGBM']    # removed 'XGBoost' because of calculation error
    predictions = [svm_predictions, rf_predictions, lgbm_predictions]               # removed xgb_predictions because of calculation error
    best_models = [svm_best_model, rf_best_model, lgbm_best_model]                  # removed xgb_best_model because of calculation error
